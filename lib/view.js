			...
			.then((results) => {
				...
				if ((!results || (utils.isArray(results) && !results.length)) && 'none' in callbacks) {
					...
					if (utils.isFunction(callbacks.then)) {
						return callbacks.then(undefined, results, next);
					} else {
...
View.prototype.render = function (renderFn, locals, callback) {
	...
	keystone.getMiddleware('pre:render').forEach((fn) => {
		preRenderQueue.push((next) => {
			fn(req, res, next);
		});
	});
	...
	async.eachSeries(this.initQueue, (i, next) => {
		if (Array.isArray(i)) {

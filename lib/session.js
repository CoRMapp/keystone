...
const postHookedSigninWithUser = (user, req, res, onSuccess, onFail) => {
	keystone.callHook(user, 'post:signin', req, (err) => {
		if (err) {
			return onFail(err);
		}
...
const doSignin = (lookup, req, res, onSuccess, onFail) => {
	if (!lookup) {
	...
	if (typeof lookup.email === 'string' && typeof lookup.password === 'string') {
		...
		User.model.findOne({ email: emailRegExp })
			.then((user) => {
				if (user) {
					user._.password.compare(lookup.password, (err, isMatch) => {
						if (!err && isMatch) {
		...
		const userId = (lookup.indexOf(':') > 0) ? lookup.slice(0, lookup.indexOf(':')) : lookup;
		const passwordCheck = (lookup.indexOf(':') > 0) ? lookup.slice(lookup.indexOf(':') + 1) : false;
		User.model.findById(userId)
			.then((user) => {
				const passwordMatch = passwordCheck && passwordCheck.length === hash(user.password).length
				&& crypto.timingSafeEqual(Buffer.from(passwordCheck), Buffer.from(hash(user.password)));
				if (user && (!passwordCheck || passwordMatch)) {
					postHookedSigninWithUser(user, req, res, onSuccess, onFail);
				} else {
...
exports.signin = (lookup, req, res, onSuccess, onFail) => {
	keystone.callHook({}, 'pre:signin', req, (err) => {
		if (err) {
			return onFail(err);
		}
...
exports.signout = (req, res, next) => {
	keystone.callHook(req.user, 'pre:signout', (err) => {
		if (err) {
		...
		req.session.regenerate((err) => {
			...
			keystone.callHook({}, 'post:signout', (err) => {
				if (err) {
...
exports.persist = (req, res, next) => {
	...
	if (keystone.get('cookie signin') && !req.session.userId && req.signedCookies['keystone.uid'] && req.signedCookies['keystone.uid'].indexOf(':') > 0) {
		exports.signin(req.signedCookies['keystone.uid'], req, res, () => {
			next();
		}, (err) => {
			const cookieOpts = _.defaults({}, keystone.get('cookie signin options'), {
				signed: true,
...
exports.keystoneAuth = (req, res, next) => {

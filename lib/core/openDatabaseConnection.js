...
module.exports = function openDatabaseConnection (callback) {
	...
	if (keystone.get('mongo replica set')) {
		...
		if (keystone.get('env') === 'development') {
			keystone.mongoose.set('debug', false);
		}
	}

	keystone.mongoose.connection.on('error', (err) => {

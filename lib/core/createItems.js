			...
			async.each(lists, (key, doneList) => {
				...
				async.each(data[key], (srcData, doneItem) => {
					...
					async.each(relationships, (field, doneField) => {
						...
						if (typeof fieldValue === 'function') {
							...
							if (field.many) {
								...
								const refsArr = _.compact(fieldValue.map((ref) => {
									return isMongoId(ref) ? ref : refsLookup && refsLookup[ref] && refsLookup[ref]._id;
								}));
							...
							const refItem = isMongoId(fieldValue) ? fieldValue : refsLookup && refsLookup[fieldValue] && refsLookup[fieldValue]._id;
						...
						} else if (fieldValue && fieldValue._id) {

							relationshipsUpdated++;
							doc.set(field.path, field.many ? [fieldValue._id] : fieldValue._id);
							doneField();

...
function Storage (options) {
	...
	for (const fieldPath in schemaFields) {
		if (!schemaFields[fieldPath]) continue;

		const type = AdapterType.SCHEMA_TYPES[fieldPath] || SCHEMA_TYPES[fieldPath];
		if (!type) throw Error(`Unknown type for requested schema field ${fieldPath}`);
		schema[fieldPath] = type;
	}
...
const normalizeFile = (file, schema, callback) => {
...
Storage.prototype.uploadFile = function (file, callback) {
	...
	normalizeFile(file, this.schema, (err, file) => {
		if (err) return callback(err);
		self.adapter.uploadFile(file, (err, result) => {
			if (err) return callback(err);

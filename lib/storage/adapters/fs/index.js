...
FSAdapter.prototype.getFileURL = function (file) {
	...
	return publicPath.endsWith('/') ? publicPath + file.filename : publicPath + '/' + file.filename;
};
...
FSAdapter.prototype.uploadFile = function (file, callback) {
	...
	this.getFilename(file, (err, filename) => {
		...
		fs.move(file.path, uploadPath, fsOptions, (err) => {
			if (err) return callback(err);
...
FSAdapter.prototype.removeFile = function (file, callback) {
	...
	fs.unlink(this.pathForFile(file.filename), (err) => {
...
FSAdapter.prototype.fileExists = function (filename, callback) {
	const filePath = this.pathForFile(filename);

			...
			.then((updateRecord) => {
				if (!updateRecord) {
					...
					if (_.isObject(update.create)) {
						...
						const background_mode = update.__background__
							? ' (background mode) '
							: '';

						update = (done) => {
							keystone.createItems(items, ops, (err, stats) => {
								if (!err) {
						...
						update((err) => {
							if (!err) {
								if (update.__commit__ !== false) {
									new Update({ key: file }).save().catch((saveErr) => {
										console.error(`Error saving update record for ${file}:`, saveErr);
									});
								}
							} else {
								done(err);

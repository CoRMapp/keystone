...
function updateItem (item, data, options, callback) {
	...
	function doFieldValidation (field, done) {
		...
		field.validateInput(data, (valid, detail) => {
			if (!valid) {
				...
				{
					field.validateRequiredInput(item, data, (valid, detail) => {
						if (!valid) {
	...
	async.series([
		/* Process validation */
		(doneValidation) => {
			async.each(fields, doFieldValidation, () => {
		...
		(doneUpdate) => {
			async.each(fields, doFieldUpdate, () => {
	...
	(err) => {
		if (err) {
			if (err instanceof Error) {
				...
				// Temporary catch custom mongoose validation errors here

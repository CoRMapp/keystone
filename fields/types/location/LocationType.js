...
class location extends FieldType {
	...
	constructor (list, path, options) {
		...
		if (options.required) {
			if (Array.isArray(options.required)) {
				...
				options._requiredPaths = options.required;
			} else if (typeof options.required === 'string') {
				// or it can be specified as a comma-delimited list
				options._requiredPaths = options.required.replace(/,/g, ' ').split(/\s+/);
			}
		...
		this.requiredPaths = options._requiredPaths || ['street1', 'suburb'];
	}
	...
	format (item, values, delimiter) {
		...
		const paths = this.paths;
		values = values.split(' ').map((i) => item.get(paths[i]));
	...
	updateItem (item, data, callback) {
		...
		if (!values) {
			...
			valuePaths = valueKeys.map((i) => paths[i]);
		...
		if (doGoogleLookup) {
			...
			this.googleLookup(item, false, googleUpdateMode, (err, location, result) => {
	...
	googleLookup (item, region, update, callback) {
		...
		doGoogleGeocodeRequest(address, region || keystone.get('default region'), (err, geocode) => {
			...
			_.forEach(result.address_components, (val) => {
